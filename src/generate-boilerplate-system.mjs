...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'mist-engine', 'mist-engine', and 'mist-engine'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.mist-engine/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.mist-engine/g),
   ...
   * Rename files.
   *
   * Rename files that had mist-engine in their name, such as
   * css/mist-engine.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*mist-engine*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('mist-engine', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-mist-engine-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob mist-engine's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace mist-engine name mentions in files.
      generator.replaceFileContents();
      // Rename files that had mist-engine in their name.
